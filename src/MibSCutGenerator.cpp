...
MibSCutGenerator::bilevelFeasCut1(BcpsConstraintPool &conPool)
{
   ...
   if(!bS->isIntegral_) return 0; // ? check the parameter meaning 

   bS->isIntegral_ = false;

   /** Find binding constraints at current solution **/
   int * binding = getBindingCons();
   /* // YX: for debug display
   int ctr = 0; 
   std::cout << "binding cons:";
   for (auto it = binding; ctr < numRows + numCols; it++)
   {
      std::cout<< *it << " ";
      ctr++;
   }
   ...
   */
   ...
   /* YX: Handling RHS; moved to after relative-prime checking*/
   /* 
   ...
   */
   ...
   double * tempVals = new double[numCols]; // YX: allocate memory; same size as var num 
   CoinFillN(tempVals, numCols, 0.0); // YX: fill the array(its size) with given value 
   double value(0.0);
   int mult(0);
   ...
   // std::cout<< "Coeffs: "; // YX: for debug only
   for(i = 0; i < numCols; i++){ 
      for(j = 0; j < numRows; j++){
         value = matrix->getCoefficient(j, i); //return one element of packed matrix
         switch(rowsense[j]){
         case 'L':
         mult = 1;
         break;
      ...
      // now tempVal is sum_j(a_ji) for constraints; adding var bounds in next lines
      // YX: add mutiplier for co-prime check 
      if (localModel_->multiplier != 1)
      {		
         tempVals[i] += binding[numRows + i] * (double)localModel_->multiplier; // variable upper bound
         tempVals[i] += - binding[numRows + numCols + i] * (double)localModel_->multiplier; // variable lower bound
      }
      else
      {
      ...
      if((tempVals[i] > etol) || (tempVals[i] < - etol)){
         ...
         //std::cout<< tempVals[i] << " "; // YX: for debug only

      }
   }
    // std::cout<< "\n"; // YX: for debug only
   // YX: to test co-prime check
   //----*----*----*----*----*----*----*----*----
   if (localModel_->cutgcdcheck == true)
   {
      ...
      */
      //std::cout<< "\n";
      std::cout<< "Original Upper "<< upper << std::endl;
      upper = upper/(double)gcdivisor;
      //std::cout<< "Modified Upper "<< upper << std::endl;
   }
   //----*----*----*----*----*----*----*----*----
   ...
   // YX: strengthened by comparing ubs; check bound cut and condition
               ...
               double *optLowerSol, const double *sol, bool &isTimeLimReached)
{
   ...
   double targetGap(localModel_->MibSPar_->entry(MibSParams::slTargetGap));  // YX: target LL optimality gap 
   double etol(localModel_->etol_);
   ...
   int newNumRows = (targetGap > etol)? lRows + 2 : lRows + 1; // YX: if target gap > 0, add a new constraint
             ...
             const double* lpSol, std::vector<double> &alphaVec)
{
   ...
   else { // YX: NOTE: rhs[lRows] = 0 will lead to alpha = 0; cut cannot be generated
      for(i = 0; i < lCols; i++){ // YX: with gap, type I IC: add gap to lower obj d^2y^*; NOTE: when  
         colIndex = lColInd[i];
         rhs[lRows] += objSense * lObjCoeffs[i] * lpSol[colIndex];
         templObj += objSense * lObjCoeffs[i] * lowerSolution[i];  // YX: lowerSolution[i] is bilevel feasible y^*; using VF in BR
      }
      if (templObj/(1 - targetGap/100) > etol){
         rhs[lRows] -= templObj/(1 - targetGap/100); // YX: if lower obj > 0
      }
      else{
         rhs[lRows] -= templObj/(1 + targetGap/100); // YX: if lower obj <= 0
      } 
   ...
   for (i = 0; i < numNonBasic; i++){
      ...
      alphaVec[i] = alpha;
      // YX: temporarily modified; only true when all alpha > 0 since used as divisors
      // YX: intersectionFound is reset for each i?
      // YX: ANY good alpha > -etol, good enough; MODIFY ALPHA > 0 IN NEXT SECTION
      if(alpha > etol){ // alpha > -1 * etol
         intersectionFound = true;
      } else if  (alpha > -1 * etol){
         intersectionFound = false;
         std::cout << "IC: alpha ~= 0; skip generation." << std::endl;
         break;
      }
   }
                  ...
                  double *optLowerSol, double* lpSol, bool &isTimeLimReached)
{
   ...
   // int newNumRows(2 * lRows + 2 * lCols + 1); // YX: if target gap > 0, newNumRows+1 
   int newNumRows = (targetGap > etol)? (2 * lRows + 2 * lCols + 2) : (2 * lRows + 2 * lCols + 1); 
   double lObjSense(localModel_->getLowerObjSense());
   double *lObjCoeff(localModel_->getLowerObjCoeffs());
   int *lColInd(localModel_->getLowerColInd());
   int *lRowInd(localModel_->getLowerRowInd());
   ...
   //YX: if targetGap, upperbound of the constraint: d^2\yhat - d^2y^*(1+gap)
   if (targetGap > etol){
      for (i = 0; i < lCols; i++){ 
         colIndex = lColInd[i];
   ...
   */
   ...
   if(1){//if(colStatus[i] != 1){ 
     ...
     if((upSlack > - etol) && (upSlack < etol)){
      ...
      binding[index] = 1;
      // YX: added if condition for coeff co-prime check
      if (localModel_->multiplier != 1)
      {
         upper += colub[i] * (double)localModel_->multiplier;
      }
      else
      {
         upper += colub[i]; // before adding if condition
      }
      bindingCons++;
     }
      ...
      if (localModel_->multiplier != 1)
      {
         upper -= collb[i] * (double)localModel_->multiplier;
      }
      else
      {
         upper -= collb[i]; // before adding if condition
      }
      bindingCons++;
     }
   }
...
//#############################################################################
int MibSCutGenerator::checkCutGCD(std::vector<double> coeff, double rhs, int size)
{
   MibSModel mdl;
   // for future: check integers here 
   int divisor = abs((int) rhs);
   //int divisor = (int) rhs;
   for (int i = 0; i < size; i++)
   {
      divisor = mdl.greatestCommonDivisor(divisor, abs((int) coeff[i])); // use gcd finder in MibSModel file
      //divisor = mdl.greatestCommonDivisor(divisor, (int) coeff[i]);
      // if found 1, skip the rest and return 1?
      if (divisor == 1)
      {
         return 1;
      }
