				 ...
				 const double* lpSol, std::vector<double> &alphaVec)
{
	...
	if ((targetGap < etol) || (uselessIneqs)){ // YX: no optimality gap or type II IC
		...
		for(i = 0; i < lCols; i++){ // YX: with gap, type I IC: add gap to lower obj d^2y^* 
			colIndex = lColInd[i];
			rhs[lRows] += objSense * lObjCoeffs[i] * lpSol[colIndex];
			templObj += objSense * lObjCoeffs[i] * lowerSolution[i];  // CHECK lowerSolution[i] is bilevel feasible y^*
		}
		if (templObj/(1 - targetGap/100) > etol){
