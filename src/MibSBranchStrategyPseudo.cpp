...
MibSBranchStrategyPseudo::createCandBranchObjects(int numPassesLeft, double ub)
{
   ...
   //       a "feasible" solution.
   //--------------------------------------------------
   // YX: QUICK FIX: pass < 3; pass < 2, use the original setting; pass == 2, forcing Linking strategy
   for (pass = 0; pass < 3; ++pass) { // pass < 2
      ...
      if ((branchPar == MibSBranchingStrategyFractional) && (pass > 1)){
         branchPar = MibSBranchingStrategyLinking;
         std::cout <<"PSEUDO: no fractional variable; forcing linking strategy" << std::endl;
      }

      if(branchPar == MibSBranchingStrategyLinking){
         for (i = 0; i < uN; ++i){
            index = upperColInd[i];
            if (fabs(lower[index]-upper[index])<=etol){ // YX: find fixed UL variables
               fixedVar[index]=1;
            }
         }
         for (i = 0; i < numCols; ++i) {
            ...
            if((fixedInd[i] == 1) && (fabs(infeasibility) > etol)){  // YX: find fractional linking variables
               found = 1;
               break;
            }
         }
      }
      // YX: if continuous variable, mark 0; if integer variable, mark 2; if candidate, mark 1
      for (i = 0; i < numCols; ++i) {
         if(colType[i] == 'C'){
            candidate[i] = 0;
         }
            ...
            candidate[i] = 2;
            value = saveSolution[i];
            infeasibility = fabs(floor(value + 0.5) - value); // YX: already taken abs value; remove others?
            if(branchPar == MibSBranchingStrategyLinking){
               if((bS->isLinkVarsFixed_ == true) && (bS->isIntegral_ == false)){ // YX: if all linkings are fixed
                  if(fabs(infeasibility) > etol){
                     candidate[i] = 1;
                  }
               }
               else if((fixedInd[i] == 1) && // YX: if is linking && ((not fractional && not fixed) || fractional)
                  (((found == 0) && (fixedVar[i] != 1)) ||
      ...
      if (numInfs) {
         ...
         if (roundAgain) {
            ...
            if (!solver->isProvenOptimal()) {
               ...
               bStatus = -2;
               std::cout << "PSEUDO: terminated with infeasible resolve" << std::endl;
               goto TERM_CREATE;
            }
            else {
   ...
   } // EOF 2 pass; YX: modified for at most 3 passes in fractional branching
