...
def parseInstanceOutput(o_entry, d_entry, results, keywords, opt_values, etol):
    ...
    # Pre-pending IDBC to differentiate from stable MIBS versions
    results['IDBC_num_int_idic'].append(0)
    results['IDBC_num_frac_idic'].append(0)
    results['IDBC_ls_cpu'].append(0)
    results['IDBC_milp_cpu'].append(0)
    results["IDBC_int_idic_called"].append(0)
    results["IDBC_int_idic_failed"].append(0)
    results["IDBC_frac_idic_called"].append(0)
    results["IDBC_frac_idic_failed"].append(0)
    results['IDBC_idic_ls_time'].append(0)
    results['IDBC_idic_milp_time'].append(0)
    ...
    with open(o_entry.path, "r") as file:
        for line in file.read().splitlines():
            ...
            #### BatRalImproving25 (ID paper)
            # feb:  since I keywords["cpu"] is a substring of 
            #       keywords["IDBC_ls_cpu"], I had to put this check before 
            #       the check for keywords["cpu"]
            elif keywords["IDBC_ls_cpu"] in line:
                results['IDBC_idic_ls_time'][-1] = float(line.split(':')[-1])
            ...
            #### BatRalImproving25 (ID paper)
            elif keywords['IDBC_num_int_idic'] in line:
                results["IDBC_int_idic_called"][-1] = (int(line.split(' ')[-1]))
                results["IDBC_num_int_idic"][-1] = (int(line.split(' ')[-5]))
                results["IDBC_int_idic_failed"][-1] = results["IDBC_int_idic_called"][-1] \
                                                        - results["IDBC_num_int_idic"][-1]
                
            elif keywords['IDBC_num_frac_idic'] in line:
                results["IDBC_frac_idic_called"][-1] = (int(line.split(' ')[-1]))
                results["IDBC_num_frac_idic"][-1] = (int(line.split(' ')[-5]))
                results["IDBC_frac_idic_failed"][-1] = results["IDBC_frac_idic_called"][-1] \
                                                        - results["IDBC_num_frac_idic"][-1]

            elif keywords["IDBC_milp_cpu"] in line:
                results['IDBC_idic_milp_time'][-1] = float(line.split(':')[-1])
...
def parseOutput(outputDir, versions, scenarios, keywords, dataSets,
    ...
    if 'idbc' in versions:
        # sum idic_milp_time+idic_ls_time -> idic_ifd_time
        df["idic_called"] = df["IDBC_int_idic_called"] + df["IDBC_frac_idic_called"]
        df["idic_ifd_time"] = df["IDBC_idic_ls_time"] + df["IDBC_idic_milp_time"]
        # compute average ifd time per idic call
        df["idic_ifd_avg_time"] = df["idic_ifd_time"] / df["idic_called"]
        df["idic_ifd_avg_time"] = df["idic_ifd_avg_time"].fillna(0)
