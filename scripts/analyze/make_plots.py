...
keywords = {
    ...
    # ========================================================
    #  BatRalImproving25: Improving Directions paper keywords
    # ========================================================
    "IDBC_num_int_idic" : "Improving direction integer calls",
    "IDBC_num_frac_idic": "Improving direction fractional calls",
    "IDBC_ls_cpu": "Local Search CPU time spent",
    "IDBC_milp_cpu": "MILP CPU time spent",
    # ========================================================
...
# Output parent path
outputDir = [params['outputDir']]   # feb: This is CLI parameter now
                                    # why is this a list??
...
#       'IBLP-ZHANG2',
    ...
    #'all'
# ]
# feb: This is CLI parameter now
dataSets = params['dataSets']

# aggregate = True
# aggregate = False
# feb: This is CLI parameter now
aggregate = params['aggregateDatasets']

aggrDataSName = ''

if aggregate:
    for i in dataSets:
        if i in ['MIBLP-XU', 'miblpXu']:
            aggrDataSName += 'X-'
        elif i in ["IBLP-FIS", 'iblpFis']:
            aggrDataSName += 'F-'
        elif i in ['INTERD-DEN', 'interdDen']:
            aggrDataSName += 'INT-'
        elif i in ['interKpShi']:
            aggrDataSName += 'SHI-'
        elif i in ['IBLP-DEN', 'iblpDen']:
            aggrDataSName += 'D-'
        elif i in ['IBLP-DEN2', 'iblpDen2']:
            aggrDataSName += 'D2-'
        elif i in ['IBLP-ZHANG', 'iblpZhang']:
            aggrDataSName += 'Z-'
        elif i in ['IBLP-ZHANG2', 'iblpZhang2']:
            aggrDataSName += 'Z2-'
        elif i in ['BENCHMARK']:
            aggrDataSName += 'BENCH-'
        else:
            aggrDataSName += i + '-'
    aggrDataSName = aggrDataSName[:len(aggrDataSName)-1]
elif len(dataSets) == 1:
    aggrDataSName = dataSets[0]


# specify file names
file_txt = "ltx_tb_cut.txt"
file_csv_out = "summary_" if aggrDataSName == '' else "summary_"+aggrDataSName
file_csv_in = params['fileCsvIn'] # file_csv_in = "/home/ted/Res/MibS/saharCutMibSPaper/rev5/summary_D-D2-Z2.csv"
