                    ...
                    with os.scandir(subDir) as output_it: 
                        for o_entry in output_it:
                            if o_entry.name.endswith('.out'):
                                ...
                                results['instance'].append(os.path.splitext(o_entry.name)[0])
                                ...
                                with open(o_entry.path,'r') as file:
                                    for line in file.read().splitlines():
                                        if keywords['solved'] in line:
                                        ...
                                        elif (keywords['nodes_proc'] in line or
                                            ...
                                            results['objval'].append(int(float(line.split('=')[1])))
                                                
                                        elif keywords['gap'] in line:
                                            incomplete = False
                                            if 'infinity' in line:
                                                results['gap'].append(1000000) # no soln found
                                                #results['solved'].append(False)
                                            else:
                                                if nosoln:
                                        ...
                                        #    results['ll_int_var'].append(int(line.split(':')[1]))
                                            ...
                                            found=True
                                            results['num_cuts'].append(int(line.split(' ')[8]))
                                            results['cut_time'].append(float(line.split(' ')[12]))
                                ...
                                if incomplete:
                                    ...
                                    print("Incomplete instance:", o_entry.name, scenario)
                                elif nosoln:
                                    results['vf_solved'].append(-1)
                                    results['ub_solved'].append(-1)
                                    results['vf_time'].append(-1)
    ...
    for inst in instList:
        ...
        for scn in scnList:
            for v in versionList:
                cond = (df['scenario'] == scn) & (df['instance'] == inst) & (df['version'] == v)
                df_temp = df[cond]
...
def dropFilter(df, scenarios, ds):
    ...
    df_time = df.xs((ds, 'cpu_time'), level=['datasets', 'fields'], axis=1,
                    drop_level=True).copy()
    df_solved = df.xs((ds, 'solved'), level=['datasets', 'fields'], axis=1,
                    drop_level=True).copy()
    print(df_solved)
    ...
    df_gap = df.xs((ds, 'gap'), level=['datasets', 'fields'], axis=1,
                   drop_level=True).copy()
...
def plotPerfProf(df, plotname='perf_profile', fixmin=None, xmin=1, xmax=None, legendnames={}):
    """
    ...
    ax.legend(loc='upper left', bbox_to_anchor = (0.6, 0.9), markerscale=1.25, frameon=True,
...
def plotCum(df, plotname='cum_profile', legendnames={}):
    ...
    ax[0].legend(loc='upper left', markerscale=1.25, frameon=True,
                   labelspacing=0.35, fontsize='x-small')
...
if __name__ == "__main__":
    ...
    #if len(args) == 0:
    if (1):
        df_r = parseOutput(outputDir, versions, scenarios, writeCSV=False,
                           filename=file_csv)
    else:
        try:
    ...
    for ds in dataSets:
        ...
        for col in plotCols:
            df_sub = df_solved.xs((ds, col), level=['datasets', 'fields'], axis=1,
                                  drop_level=True).copy()
            plotPerfProf(df_sub, plotname = 'perfprof_'+col+'_'+ds, xmax=plotCols[col][1])
        plotCum(df_has_soln)
